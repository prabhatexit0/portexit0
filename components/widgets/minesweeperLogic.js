...
const isValidCoordinate = (i, j, board) =>
  i >= 0 && i < board.length && j >= 0 && j < board[0].length

const addBombsAndMark = (x, y, board) => {
  const mutableBoard = [...board]
  let m = mutableBoard.length,
    n = mutableBoard[0].length
...
const revealAroundZero = (mutableBoard, i, j) => {
  let rows = mutableBoard.length,
    columns = mutableBoard[0].length
  let vis = Array(rows)
    .fill(null)
...
const revealAllBombs = mutableBoard => {
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < columns; j++) {
      if (mutableBoard[i][j].isBomb) mutableBoard[i][j].isClicked = true
    }
  }
}
